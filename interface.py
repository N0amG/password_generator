...
class Menu1(ctk.CTk):
    ...
    def menu2(self):
        ...
        self.password_manager.sort_passwords()
        ...
        self.scrollable_frame = ctk.CTkScrollableFrame(self, label_text="Passwords History", label_font=self.font)
        ...
        self.search_entry.place(relx=0.35, rely=0.02, relwidth = 0.25)

        self.search_button = ctk.CTkButton(self, text="Rechercher", command=self.search)
        self.search_button.place(relx=0.7, rely=0.02, anchor=ctk.NE)
        
        self.search_entry.bind("<KeyRelease>", self.search)
        
        self.frame_display()
    
    def frame_display(self, search_dict = False):
        print("ici", search_dict)
        self.page_label = ctk.CTkLabel(self, text=f"Page {self.page+1} / {ceil(self.passwords_numbers/self.page_coef)}")
        self.page_label.place(relx = 0.26, rely= 0.02, anchor=ctk.NE)
        ...
        for i in range(self.page_coef*self.page, self.page_coef + self.page_coef*self.page):
            ...
            if not search_dict: 
                ...
                if search_dict != [] and i <= len(search_dict):
                    condition = True
    ...
    def previous_page(self, page=False):
        if self.page > 0:
            self.page -= 1
            if page: self.page = page
            self.page_label.configure(text=f"Page {self.page+1} / {ceil(self.passwords_numbers/self.page_coef)}")
            for box in self.box_list:
                box.destroy()    
            if page == False: self.frame_display()
    ...
    def search(self):
        ...
        results = []  # Stocker les résultats de la recherche
        
        # Parcourir vos données
        for item in self.dict_list:  # Remplacez self.data par votre propre liste de données
            if search_term in item["website"].lower() or search_term in item["username"].lower():
                results.append(item)
        
        # Mettre à jour l'affichage avec les résultats de la recherche
        self.previous_page(page=0)
        self.frame_display(results)
